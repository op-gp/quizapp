...
import { getQuizPlay, submitQuiz } from "../api/student.ts";
...
export default function QuizPlay() {
  ...
  const handleAutoSubmit = async () => {
    ...
    try {
      ...
      console.error(err);
      alert(
        "Time expired! Quiz was submitted, but some answers could not be evaluated."
      );
      navigate("/dashboard");
    }
  };
    ...
    return (
      <div className="flex h-screen w-screen flex-col items-center justify-center bg-slate-950 p-4">
        <Alert className="max-w-md bg-red-500/10 border-red-500/20 text-red-400 mb-6">
          <AlertDescription>
            {error || "Unable to retrieve quiz play details."}
          </AlertDescription>
        </Alert>
        <Button
            ...
            <Card className="border-slate-900 bg-slate-900/20 backdrop-blur-md p-6 space-y-6">
              ...
              <div className="space-y-3">
                {currentQuestion.options.map((opt) => {
                  const isChecked =
                    selectedAnswers[currentQuestion._id] === opt._id;
